...
| `gateway` _[GatewayConfig](#gatewayconfig)_ | Gateway configuration for OpenDataHub platform-wide gateway.<br />This configures authentication, certificates, and domain settings for the gateway. |  |  |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `oidc` _[OIDCClientConfig](#oidcclientconfig)_ | OIDC client configuration (used when cluster is in OIDC auth mode) |  |  |
...
| `clientSecretRef` _[SecretKeySelector](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.25/#secretkeyselector-v1-core)_ | Reference to secret containing clientID and clientSecret for gateway OIDC client |  |  |
